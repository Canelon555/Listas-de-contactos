...
list.addEventListener('click', e => {
  ...
  if (editBtn) {
    ...
    const validName = NAME_REGEX.test(nameEdit.innerHTML);
    const validNumber = NUMBER_REGEX.test(numberEdit.innerHTML);
    ...
    if (li.classList.contains('editando')) {
      ...
      if (!validName && !validNumber) {
        ...
        alert('Nombre y Numero incorrectos, intente nuevamente');
        return;
      } else if (!validName && validNumber) {
        nameEdit.classList.add('edit-error');
        numberEdit.classList.remove('edit-error');
        alert('Nombre incorrecto, los nombres deben empezar con letra mayuscula');
        return;
      } else if (!validNumber && validName) {
        nameEdit.classList.remove('edit-error');
        numberEdit.classList.add('edit-error');
        alert('Numero de Telefono incorrecto, usar un numero telefonico venezolano vaido');
        return;
      }
